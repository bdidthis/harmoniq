...
class _OfflineFileAnalyzerPageState extends State<OfflineFileAnalyzerPage> {
  ...
  Future<void> _analyze() async {
    ...
    try {
      ...
      if (!ReturnCode.isSuccess(rc)) {
        final logs = await sess.getLogsAsString();
      ...
      final dataBytes = totalBytes - dataOffset;
      if (dataBytes <= 0) {
        throw Exception('Invalid WAV file: no data');
      }

      final stream = file.openRead(dataOffset);
      int processed = 0;
      ...
      final sub = stream.listen((chunk) {
        ...
        if (bytes.length.isOdd) bytes = bytes.sublist(0, bytes.length - 1);
      ...
      // 4) Collect final results (respect calibrated bias + resolver in page)
  ...
  Future<void> _exportLogs() async {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Logs exported (CSV + JSON)')));
  }
