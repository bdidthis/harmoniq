...
class TestLogEntry {
  ...
  Map<String, dynamic> toJson() => {
        ...
        'hpcp_bins': hpcpBins,
        'custom_params':
            customParams != null ? json.encode(customParams) : null,
      };
  ...
  String toCsv() {
    final values = [
      ...
      hpcpBins?.toString() ?? '',
      customParams != null ? json.encode(customParams) : '',
    ];
...
class HarmoniQLogger {
  ...
  void _logToConsole(TestLogEntry entry) {
    ...
    output.writeln('╠════════════════════════════════════════╣');
    output.writeln('║ Test: ${entry.testId.padRight(33)}║');
    output.writeln('║ Type: ${entry.testType.name.padRight(33)}║');
